...
Control.Window = Class.create({
    ...
    position: function(event){
        ...
        if (window.orientation && window.devicePixelRatio > 1) {
            viewport_dimensions.width = viewport_dimensions.width / window.devicePixelRatio;
            viewport_dimensions.height = viewport_dimensions.height / window.devicePixelRatio;
        }
        Position.prepare();
        ...
        if(this.options.position == 'center' || this.options.position == 'center_once'){
            var top = (container_dimensions.height <= viewport_dimensions.height) ? ((offset_top != null && offset_top > 0) ? offset_top : 0) + 'px' : 0
            this.container.setStyle({
                top: '20px',
